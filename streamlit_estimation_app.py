...
# Measurement Sheets Page
        ...
        with st.form("add_measurement"):
            ...
            if submitted and description.strip():
                new_measurement = {
                    ...
                    'unit': unit,
                    'total': total,
                    'rate': auto_rate if selected_ssr != "Select SSR Code..." else 0,
                    'estimated_cost': total * auto_rate if selected_ssr != "Select SSR Code..." else 0
                }
                ...
                if selected_ssr != "Select SSR Code...":
                    st.success(f"✅ Measurement added with SSR Code: **{selected_ssr}** | Estimated Cost: **₹{total * auto_rate:,.2f}**")
                else:
    ...
    if not st.session_state.measurements.empty:
        ...
        # Add SSR and cost columns if they exist
        if 'ssr_code' in display_df.columns:
            display_columns["ssr_code"] = "SSR Code"
        if 'rate' in display_df.columns:
            display_columns["rate"] = st.column_config.NumberColumn("Rate (₹)", format="%.2f")
        if 'estimated_cost' in display_df.columns:
            display_columns["estimated_cost"] = st.column_config.NumberColumn("Est. Cost (₹)", format="%.2f")
